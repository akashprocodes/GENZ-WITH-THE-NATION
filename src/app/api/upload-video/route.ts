...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    logger.info('Video upload verification requested', { fileId, requestId });
    ...
    // 2. Create Video Submission
    const submission = await DatabaseServiceProvider.createVideoSubmission!({
      name: user.name,
      email: user.email,
      mobile: user.mobile,
    ...
    return NextResponse.json({
      success: true,
      message: 'Video upload completed and verified',
      data: submission,
    ...
    logger.error('Video upload verification/submission failed', { error, requestId });
    return NextResponse.json({
      success: false,
